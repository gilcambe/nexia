...
function renderSite() {
  ...
  root.innerHTML = `
  <section id="hero-sec"><div style="position:absolute;top:50%;left:50%;width:120vw;height:120vh;transform:translate(-50%,-50%);border:none;opacity:0.3;pointer-events:none;" allow="autoplay;encrypted-media" allowfullscreen></div>
    </div>
...
window.openLightbox = function(idx) {
  ...
  if (lbImg) { lbImg.src = g.src; lbImg.alt = caption; }
  if (lbCaption) lbCaption.textContent = caption;
  if (lbBox) lbBox.classList.add('open');
  document.body.style.overflow = 'hidden';
};
...
function _chatAddMsg(role, text) {
  ...
  const av = role === 'bot'
    ...
    : ''
  div.innerHTML = `${av}<span class="chat-text">${text}</span>`;
