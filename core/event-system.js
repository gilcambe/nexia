    ...
    while (_queue.length && !_paused) {
      ...
      for (const handler of handlers) {
        try {
          ...
          if(window._nexiaDebug) console.warn(`[EVENT SYSTEM] Handler error for ${event.type}:`, 'Internal error');
          _persistEvent({ ...event, type: 'system:error', data: { originalEvent: event.type, error: 'Internal error' } });
        }
      }
...
const NexiaLogger = (() => {
  ...
  async function _flushBuffer() {
    ...
    try {
      ...
      console.warn('[LOGGER] Flush failed:', 'Internal error');
    }
  }
