...
(function () {
  ...
  win.id = 'nsa-window';
  win.innerHTML = `<div id="nsa-header"><span>${agentName}</span><button onclick="nsaToggle()" style="background:none;border:none;color:inherit;cursor:pointer;font-size:18px">✕</button></div><div id="nsa-msgs"></div><div id="nsa-input-row" style="display:flex;gap:8px;padding:10px;border-top:1px solid var(--brd,#222)"><input id="nsa-input" placeholder="Digite sua mensagem..." style="flex:1;padding:8px 12px;border-radius:8px;border:1px solid var(--brd,#333);background:var(--bg2,#111);color:var(--text,#fff);font-size:13px" onkeydown="if(event.key==='Enter')nsaSend()"><button onclick="nsaSend()" style="padding:8px 14px;border-radius:8px;background:var(--gold,#DAA520);border:none;color:#fff;cursor:pointer">➤</button></div>`;
  ...
  function appendMsg(text, role) {
    ...
    div.innerHTML = `<span>${text}</span><span class="nsa-time">${time}</span>`;
  ...
  function showTyping() {
    ...
    div.innerHTML = '<span class="nsa-dots">●●●</span>';
    msgs.appendChild(div);
    msgs.scrollTop = msgs.scrollHeight;
  }
