...
const NexiaTenantEngine = (() => {
  ...
  async function detect() {
    ...
    // Unknown tenant — fetch from Firestore ONLY when authenticated.
    // Calling Firestore without auth causes "Missing or insufficient permissions" warnings.
    if (typeof firebase !== 'undefined' && firebase.apps?.length) {
      const currentUser = firebase.auth ? firebase.auth().currentUser : null;
      if (currentUser) {
        try {
          // FIX: timeout 3s evita travamento de 10s quando Firestore offline
          const snap = await Promise.race([
            firebase.firestore().collection('tenants').doc(id).get(),
          ...
          console.warn('[TenantEngine] Firestore:', 'Internal error');
        }
      }
    }
...
const NexiaModuleEngine = (() => {
  ...
  async function loadForTenant(tenantId) {
    ...
    // Only query Firestore when authenticated — unauthenticated reads trigger "Missing or insufficient permissions"
    const _currentUser = (typeof firebase !== 'undefined' && firebase.apps?.length && firebase.auth)
      ? firebase.auth().currentUser : null;

    if (_currentUser) {
      try {
        // FIX: timeout 3s evita travamento quando Firestore offline
        const snap = await Promise.race([
          firebase.firestore().collection('tenants').doc(tenantId).collection('modules').get(),
          new Promise((_, rej) => setTimeout(() => rej(new Error('timeout')), 3000))
        ]);
        snap.forEach(d => {
        ...
        console.info('[ModuleEngine] Firestore modules indisponível, usando defaults:', 'Internal error');
        moduleIds = ['notifications', 'payments', 'analytics'];
      }
    } else {
      // Not authenticated — use default modules silently (no Firestore call)
      moduleIds = ['notifications', 'payments', 'analytics'];
    }
    ...
    for (const id of moduleIds) {
      if (_registry[id] && !_active[id]) {
        try {
          ...
          console.warn(`[ModuleEngine] Failed to init "${id}":`, 'Internal error');
        }
      }
    }
...
const NexiaThemeEngine = (() => {
  ...
  function loadAndApply(tenantId, appTarget) {
    ...
    // Apply cached immediately (zero FOUC — sem Firestore)
    try {
      const cached = localStorage.getItem(cacheKey);
      if (cached) apply(JSON.parse(cached));
    } catch (e) {}

    // FIX Auth-First: aguarda confirmação de auth antes de qualquer chamada Firestore.
    // onSnapshot sem auth causa "Missing or insufficient permissions" no console.
    if (typeof firebase === 'undefined' || !firebase.apps?.length) return;

    const _attachSnapshot = () => {
      ...
      ref.onSnapshot(snap => {
        ...
        apply(cfg);
        try { localStorage.setItem(cacheKey, JSON.stringify(cfg)); } catch (_) {}
      }, () => {}); // erro silencioso — tema do cache já foi aplicado
    };

    // FIX: só faz onSnapshot após auth confirmado
    const currentUser = firebase.auth ? firebase.auth().currentUser : null;
    if (currentUser) {
      // Já autenticado — anexa snapshot diretamente
      _attachSnapshot();
    } else if (firebase.auth) {
      // Aguarda a primeira resolução do auth state
      const unsubscribe = firebase.auth().onAuthStateChanged(user => {
        unsubscribe();
        if (user) _attachSnapshot();
        // Se não autenticado, tema do cache localStorage já está aplicado — não faz Firestore
      });
    }
  }
...
const NexiaLandingRenderer = (() => {
  ...
  function render(layout, container) {
    ...
    container.innerHTML = layout.map(block => {
      ...
      catch (e) { return `<div style="padding:20px;background:#FEF2F2;color:#991B1B;font-size:12px">Block error: ${'Internal error'}</div>`; }
    }).join('');
  }
  ...
  async function loadAndRender(tenantId, pageId, container) {
    ...
    // Show skeleton
    container.innerHTML = `<div style="padding:40px;text-align:center;color:#94A3B8">Carregando página...</div>`;
    ...
    try {
      ...
      if (!snap.exists) {
        container.innerHTML = `<div style="padding:40px;text-align:center;color:#94A3B8">Página não publicada ainda.</div>`;
        return;
      }
      ...
      console.warn('[LandingRenderer]', 'Internal error');
    }
  }
...
const NexiaRuntime = (() => {
  ...
  async function boot() {
    ...
    // Wait for Firebase Auth to initialize (prevents Firestore permission errors on boot)
    await new Promise(resolve => {
      ...
      setTimeout(resolve, 3000); // max 3s — resolve even if no auth
    });
    ...
    // 2. Apply theme
    // FIX Auth-First: loadAndApply internamente já verifica auth antes de onSnapshot.
    // Passamos o usuário atual para que não haja race condition.
    if (tenant.id && tenant.id !== 'NEXIA_MASTER') {
      NexiaThemeEngine.loadAndApply(tenant.id, 'landing');
    }

    // 3. Load active modules
    // FIX v19: NEXIA_MASTER é tenant interno — módulos são fixos, não precisam de Firestore.
    // Evita "Missing or insufficient permissions" quando usuário ainda não autenticou.
    const _authUser = (typeof firebase !== 'undefined' && firebase.apps?.length && firebase.auth)
      ? firebase.auth().currentUser : null;
    const moduleIds = (tenant.id === 'NEXIA_MASTER' && !_authUser)
      ? ['notifications', 'payments', 'analytics']
      : await NexiaModuleEngine.loadForTenant(tenant.id);
...
NexiaModuleEngine.register('analytics', {
  async init({ tenantId }) {
    // FIX Auth-First: só grava pageview se usuário autenticado.
    // Gravação sem auth viola Firestore Security Rules e gera erro 403.
    if (typeof firebase !== 'undefined' && firebase.apps?.length) {
      ...
      if (authUser) {
        try {
          ...
          // Silencioso — analytics não deve bloquear o boot
        }
      }
      // Se não autenticado: pageview não é gravado agora.
      // Será gravado quando o usuário logar (via onAuthStateChanged externo).
    }
