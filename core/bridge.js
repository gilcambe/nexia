...
const NexiaBridge = (() => {
    ...
    function syncRealtime(path, callback) {
        ...
        _listeners[key] = _buildRef(path).onSnapshot(snap => {
            ...
            _log(`syncRealtime error [${path}]: ${'Internal error'}`, 'err');
        });
    ...
    function watchMeetings(slug) {
        ...
        }, err => _log(`meetings error: ${'Internal error'}`, 'err'));
    ...
    function watchAlerts(slug) {
        ...
        }, err => _log(`alerts error: ${'Internal error'}`, 'err'));
    }
    ...
    async function audit(action, payload = {}) {
        ...
        try {
            ...
            _log(`Audit falhou: ${'Internal error'}`, 'warn');
        }
    }
