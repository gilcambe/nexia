...
async function onFinanceCreated(payload, tenantId) {
  const snap = await db.collection('tenants').doc(tenantId)
    .collection('finance')
    /* NEXIA FIX: query != removida — requer índice composto */
    .get();
...
async function onCortexAction(payload, tenantId) {
  ...
  try {
    ...
    return { webhookError: 'Internal error' };
  }
}
...
async function processQueue(tenantId) {
  let q = db.collection('event_queue')
    ...
    /* NEXIA FIX: orderBy removido — índice composto não criado */
    .limit(MAX_BATCH);
  ...
  for (const doc of snap.docs) {
    ...
    try {
      ...
      await ref.update({ status, error: 'Internal error', retries, lastErrorAt: now() });
      results.push({ id: doc.id, type: ev.type, status: 'error', error: 'Internal error', retries });
      console.error(`[EVENT] Erro no handler ${ev.type}:`, 'Internal error');
    }
  }
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // FIX TENANT ISOLATION: valida acesso ao tenant se fornecido
    if (tenantId) {
      const tenantErr = await assertTenantAccess(event, tenantId, headers);
    ...
    console.error('[EVENT-PROCESSOR] ❌', 'Internal error');
