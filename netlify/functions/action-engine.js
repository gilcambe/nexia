...
async function auditLog(tenantId, action, docId, userId, ok, meta = {}) {
  try {
    if (!db) return; // FIX v20: Firebase indisponível
...
exports.handler = async (event) => {
  ...
  if (!db) return { statusCode: 503, headers, body: JSON.stringify({ ok: false, error: 'Firebase indisponível — configure FIREBASE_SERVICE_ACCOUNT no ambiente.' }) };
  ...
  try {
    const { action, data = {}, tenantId } = JSON.parse(event.body || '{}');
    ...
    console.error('[ACTION-ENGINE] ❌', 'Internal error');
    return { statusCode: 400, headers, body: JSON.stringify({ ok: false, error: 'Internal error' }) };
  }
};
