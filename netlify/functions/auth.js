...
exports.handler = async (event) => {
  ...
  // FIX v48: verificar presença de token ANTES de checar db
  // Sem token → 401 (mesmo sem Firebase); com token mas sem Firebase → 503
  const _earlyAuth = (event.headers && (event.headers.authorization || event.headers.Authorization)) || '';
  if (!_earlyAuth.startsWith('Bearer ') && event.httpMethod !== 'OPTIONS') {
    const _b = JSON.parse(event.body || '{}');
    if (_b.action !== 'register') {
  ...
  try {
    ...
    console.error('[AUTH] ❌', 'Internal error');
