...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { action, tenantId, userId: _bodyUserId, prompt, response, intent, exampleId, rating, conversationId = 'default' } = JSON.parse(event.body || "{}");
    // FIX IDOR: userId SEMPRE do token verificado — nunca do body
    const userId = event._uid || _bodyUserId;
    ...
    console.error('[CORTEX-LEARN] ❌', 'Internal error');
    return { statusCode: 400, headers, body: JSON.stringify({ error: 'Internal error' }) };
  }
};
