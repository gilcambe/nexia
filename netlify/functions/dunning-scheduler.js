...
async function sendDunningMessage(tenant, daysPastDue, plan) {
  ...
  if (phone && process.env.WHATSAPP_TOKEN && process.env.WHATSAPP_PHONE_ID) {
    ...
    } catch (e) { console.warn('[DUNNING] WhatsApp:', 'Internal error'); }
  }
  ...
  if (email && process.env.RESEND_API_KEY) {
    ...
    } catch (e) { console.warn('[DUNNING] Email:', 'Internal error'); }
  }
...
async function alreadyActedToday(tenantId, day) {
  ...
  try {
    ...
    console.warn('[DUNNING] idempotency check failed:', 'Internal error');
  ...
  try {
    ...
    for (const doc of tenantsSnap.docs) {
      ...
      try {
        ...
        results.errors.push({ tenant: tenant.id, error: 'Internal error' });
      }
    }
    ...
    console.error('[DUNNING] Fatal:', 'Internal error');
