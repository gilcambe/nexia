...
async function planAgents(task, availableAgents) {
  try {
    ...
    if (m) {
      let parsed; try { parsed = JSON.parse(m[0]); } catch(e) { parsed = null; }
      // Limita a MAX_AGENTS
...
async function runAgent(agentKey, agent, messages, attempt = 1) {
  ...
  try {
    ...
    return { agentKey, agentName: agent.name, reply: '', actionJson: null, ok: false, error: 'Internal error' };
  }
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.error('[SWARM] ❌', 'Internal error');
