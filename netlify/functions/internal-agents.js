...
// NEXIA FIX: remove undefined values before Firestore writes (prevents 500)
...
async function qaAgent(action, payload) {
  const baseUrl = process.env.NEXIA_APP_URL || 'https://nexia.com.br';
  const tests = [];
  ...
  for (const ep of endpoints) {
    const t0 = Date.now();
    try {
      const res = await _fetchTimeout(`${baseUrl}${ep.path}`, {}, 20000, {
        method: ep.method,
        headers: { 'Content-Type': 'application/json', 'x-qa-agent': 'true' },
        body: ep.body ? JSON.stringify(ep.body) : null,
        signal: AbortSignal.timeout(5000) // REDUCED: 5s timeout per endpoint to avoid total function timeout
      });
      const latency = Date.now() - t0;
      tests.push({ name: ep.name, status: res.status, ok: res.status < 500, latency });
    } catch (e) {
      tests.push({ name: ep.name, status: 0, ok: false, latency: Date.now() - t0, error: 'Internal error' });
    }
  }
...
async function debugAgent(action, payload) {
  ...
  for (const err of errors) {
    const key = err.type || 'Internal error'?.slice(0, 50) || 'unknown';
...
Para cada erro, sugira a causa mais provável e a correção em 1-2 linhas. JSON: {"suggestions": [{"error": "...", "cause": "...", "fix": "..."}]}`
      ...
      let parsed; try { parsed = JSON.parse(text.replace(/```json|```/g, '').trim()); } catch(e) { parsed = null; }
...
async function runAllAgents() {
  const results = {};
  try { results.qa = await qaAgent(); } catch (e) { results.qa = { error: 'Internal error' }; }
  try { results.debug = await debugAgent(); } catch (e) { results.debug = { error: 'Internal error' }; }
  try { results.perf = await perfAgent(); } catch (e) { results.perf = { error: 'Internal error' }; }
  try { results.security = await securityAgent(); } catch (e) { results.security = { error: 'Internal error' }; }
