...
async function ttCall(path, params = {}, tenantCfg = {}) {
  ...
  const res = await _fetchTimeout(`${TTLOCK_BASE}${path}`, {}, 10000, {
    method: 'POST',
...
async function sendPasscodeToGuest(tenantId, phone, name, passcode, checkIn, checkOut) {
  try {
    ...
    await _fetchTimeout(`${process.env.NEXIA_APP_URL || process.env.URL || 'https://nexia.app'}/api/whatsapp`, {}, 10000, {
      method: 'POST',
    ...
    console.warn('Failed to send passcode via WhatsApp:', 'Internal error');
  }
}
