...
async function actionCreate(body, tenantId) {
  ...
  if (!isCard) {
    const payment = await mpPost('/v1/payments', {
      ...
      date_of_expiration: dueDate ? new Date(dueDate + 'T23:59:59-03:00').toISOString() : null
    });
...
async function actionCreateLink(body, tenantId) {
  ...
  const pref = await mpPost('/checkout/preferences', {
    ...
    payer: payerEmail ? { email: payerEmail } : null,
...
async function handleWebhook(event) {
  ...
  let payload;
  try { payload = JSON.parse(event.body || '{}'); } catch { return { statusCode: 400, headers, body: JSON.stringify({ error: 'JSON inválido' }) }; }
...
exports.handler = async (event) => {
  const headers = MW_HEADERS(event);
